...
MushraPage.prototype.render = function (_parent) {
  ...
  for (i = 0; i < conditions.length; ++i) {
    td = $("<td class='spaceForSlider'> \
      <span><input type='range' name='"+conditions[i].getId()+"' class='scales' value='10' min='1' max='10' data-vertical='true' data-highlight='true' style='display : inline-block; float : none;'/></span> \
    </td>");
...
MushraPage.prototype.renderCanvas = function(_parentId) {
  ...
  canvasContext.fillText("10", xTextScoreRanges, YfirstLine);
  canvasContext.fillText("8", xTextScoreRanges, prevScalesHeight * 0.2 + YfirstLine);
  canvasContext.fillText("6", xTextScoreRanges, prevScalesHeight * 0.4 + YfirstLine);
  canvasContext.fillText("4", xTextScoreRanges, prevScalesHeight * 0.6 + YfirstLine);
  canvasContext.fillText("2", xTextScoreRanges, prevScalesHeight * 0.8 + YfirstLine);
  canvasContext.fillText("1", xTextScoreRanges, prevScalesHeight + YfirstLine);
